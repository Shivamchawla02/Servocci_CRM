...
const ActionButton = ({ icon, text, color, onClick }) => (
  <button
    onClick={onClick}
    className={`${color} flex flex-col items-center justify-center gap-1 py-4 rounded-xl text-white font-medium shadow-md hover:scale-105 transition duration-200`}
  >
    {icon}
...
const AdminSummary = () => {
  ...
  const leadOrder = [
    'Qualified', 'Follow-up', 'Initiated', 'Received',
    'Documentation', 'Closed', 'Rejected', 'Unqualified'
  ];
  ...
  const inProgress =
    (leadStats["Follow-up"] || 0) +
    (leadStats["Initiated"] || 0) +
    (leadStats["Documentation"] || 0);

  const pieData = leadOrder.map(name => ({
    name,
    value: leadStats[name] || 0,
  }));
  ...
  return (
    <div className="p-4 sm:p-6 max-w-screen-xl mx-auto">
      ...
      <div className="flex justify-between items-center mb-6">
        <h3 className='text-2xl sm:text-3xl font-extrabold text-[#001b48]'>
...
>
  ...
  {translations[lang].switch}
      ...
      {/* Pie Chart */}
      <div className="mt-6 bg-white rounded-xl shadow-lg p-4 w-full h-[350px]">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            <Pie
              ...
              dataKey="value"
              label={({ name, percent }) =>
                `${translations[lang][name]} (${(percent * 100).toFixed(0)}%)`
              }
            >
      ...
      {/* Bar Chart */}
      <div className="hidden md:block w-full h-[350px] mt-10 bg-white rounded-xl shadow-lg p-4">
        <ResponsiveContainer width="100%" height="100%">
